...
// Exactly the same as GetNextAlignment() except one piece
// of character data is filled in: the alignment name.
bool BamReaderPrivate::GetNextAlignmentWithName(BamAlignment &bAlignment) {
	if (GetNextAlignmentCore(bAlignment)) {
		// Shamelessly copy/pasted from BuildCharData().
        	int seqDataOffset = bAlignment.SupportData.QueryNameLength +
                	(bAlignment.SupportData.NumCigarOperations * 4);
        	int qualDataOffset = seqDataOffset +
                	(bAlignment.SupportData.QuerySequenceLength+1)/2;
        	const char* allCharData =
			bAlignment.SupportData.AllCharData.data();
        	const char* qualData =
			((const char*)allCharData) + qualDataOffset;

		/* Saves the name of the alignment */
		bAlignment.Name.assign((const char*)(allCharData));
